...
import {
  useCreateSetupSignature, useUpdateSetupSignature, useDeleteSetupSignature,
  useSeedDefaultSetupSignatures,
} from "@/hooks/use-project-setup";
import type { ProjectSetupSignature } from "@shared/schema";
import { LABEL_CLASS, SaveStatusPill } from "./fields";
  ...
  return (
    <div>
      ...
      <Input
        ...
        data-testid={`setup-sig-${sig.id}-${field}`}
      />
    </div>
  );
}

function SigNotes({ sig, save }: { sig: ProjectSetupSignature; save: SaveSig }) {
  const { value, setValue } = useDebouncedSave<string>(
    sig.notes ?? "",
...
export function SignaturesTab({ signatures, projectId }: {
  ...
  const update = useUpdateSetupSignature(projectId);
  const remove = useDeleteSetupSignature(projectId);
  const seedDefaults = useSeedDefaultSetupSignatures(projectId);

  const [adding, setAdding] = useState(false);
  const [newRole, setNewRole] = useState("");

  const save: SaveSig = (id, patch) => update.mutateAsync({ id, ...patch });
  ...
  if (signatures.length === 0) {
    return (
      <EmptyState
        message="No sign-off block yet. The Charter falls back to the five default roles until this is filled in."
        action={
          <Button onClick={() => seedDefaults.mutate()} disabled={seedDefaults.isPending} data-testid="setup-seed-signers">
            {seedDefaults.isPending ? "Adding signers…" : "Seed default 5 signers"}
          </Button>
        }
      />
    );
      ...
      {adding ? (
        <div className="flex items-end gap-2 rounded-lg border border-dashed border-border p-3">
          <div className="flex-1">
            ...
            <Input
              autoFocus value={newRole} placeholder="e.g. Owner Representative"
        ...
        <Button variant="outline" size="sm" onClick={() => setAdding(true)} data-testid="setup-add-signer">
